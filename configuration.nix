...
let
  user      = "admin";
  password  = "password";
  hostname  = "690176eb8afcafa44bbc4acc";
  homeDir   = "/home/${user}";
  ...
  webrtcLauncher = pkgs.writeShellApplication {
    ...
    text = ''
      set -euo pipefail
      # nounset-safe init (works with `set -u`)
      : "${PYTHONPATH:=}"
      : "${PATH:=}"
      shopt -s nullglob
      ...
      echo "=== Checking ros2launch metadata visibility ==="
      "$PYBIN" - <<'PY'
      import sys, os, glob, importlib.metadata as meta
      ...
      # Check if ros2launch can be imported at all
      try:
          import ros2launch
          print("✅ Imported ros2launch from:", ros2launch.__file__)
      except Exception as e:
          print("❌ Could not import ros2launch:", e)

      # Search for egg/dist info entries
      found = []
      for path in sys.path:
          for d in glob.glob(os.path.join(path, "ros2launch-*.*-info")):
              found.append(d)

      if found:
          print("✅ Found ros2launch metadata:")
          for f in found:
              print("   ", f)
      else:
          print("❌ No ros2launch .egg-info or .dist-info found on sys.path")

      # Show whether the 'launch' verb is registered
      try:
          eps = [e.name for e in meta.entry_points(group="ros2cli.command")]
          print("Available ros2 verbs:", sorted(set(eps))[:25], "…")
          print("✅ 'launch' verb found:", "launch" in eps)
      except Exception as e:
          print("⚠️ Couldn't read entry points:", e)
      PY
      echo "==============================================="
